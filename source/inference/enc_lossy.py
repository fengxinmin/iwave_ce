...
from utils import img2patch, img2patch_padding, rgb2yuv, yuv2rgb, find_min_and_max, model_lambdas, qp_shifts
import Model
...
def enc_lossy(args):
    ...
    logfile = open(args.log_dir + '/enc_log_{}.txt'.format(args.img_name[0:-4]), 'a')
    ...
    if args.model_qp > 12.5: # 13-->26 Perceptual models
        checkpoint = torch.load(args.model_path + '/' + str(model_lambdas[args.model_qp]) + '_percep.pth')
    else: # 0-->12 MSE models
        checkpoint = torch.load(args.model_path + '/' + str(model_lambdas[args.model_qp]) + '_mse.pth')

    all_part_dict = checkpoint['state_dict']

    models_dict = {}

    if model_lambdas[args.model_qp] < 0.10 + 0.001:
    ...
    with torch.no_grad():
        ...
        LL, HL_list, LH_list, HH_list, used_scale = models_dict['transform'].forward_trans(input_img_v)
